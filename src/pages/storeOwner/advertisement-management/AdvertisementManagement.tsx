...
import { deleteAdvertisement, fetchAllAdvertisement } from '../../../api/storeApi';
...
export default function AdvertisementManagement() {
    ...
    const { isPending, error, data } = useQuery({
        ...
        queryFn: () => fetchAllAdvertisement(storeOwnerToken),
    });
    ...
    const { mutateAsync, isPending: isDeleting } = useMutation({
        mutationFn: (id: string) => {
            return deleteAdvertisement(storeOwnerToken, id);
        },
        onSuccess: () => {
            const res: any = fetchAllAdvertisement(storeOwnerToken);
    ...
    const toggleStatus = async (message: string, confirmButtonText: string, id: string) => {
        Swal.fire({
            ...
            if (result.value) {
                   await deleteAdvertisement(storeOwnerToken,id)
    ...
    //         const response: any = await updateAdvertisementDisplay(storeOwnerToken, values);
    //         showAlert('success', response?.data?.message);

    //         const res = await fetchAllAdvertisement(storeOwnerToken);
    //         dispatch(setAdvertisementDetails(res));
    //         setUpdatingModal('');
    //         setLoading(false);
    //     } catch (error) {
    ...
    return (
        <div>
            <Breadcrumbs heading="Add Advertisement" links={[{ name: 'Dashboard', href: '/admin' }, { name: 'Advertisement' }]} />
